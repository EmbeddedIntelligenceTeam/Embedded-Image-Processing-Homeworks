...
<title>YOLO Digit Detection</title>
...
#img{width:100%;border-radius:8px;border:2px solid #ff6b6b}
.btn{width:100%;padding:15px;background:linear-gradient(90deg,#ff6b6b,#ffa502);border:none;border-radius:8px;font-size:16px;font-weight:bold;cursor:pointer;color:#000;margin:5px 0}
.btn-continue{background:linear-gradient(90deg,#00d9ff,#00ff88);display:none}
.stats{display:flex;justify-content:space-around;text-align:center}
.stat{padding:10px}
.stat-val{font-size:24px;color:#ff6b6b;font-weight:bold}
.det{background:#ff6b6b33;padding:10px;margin:5px 0;border-radius:6px;border-left:3px solid #ff6b6b;display:flex;justify-content:space-between}
#frame{color:#888;font-size:12px;text-align:center}
.footer{text-align:center;margin-top:20px;color:#666;font-size:12px}
</style></head><body>
<div class="container">
<h1>YOLO Digit Detection</h1>
<div class="subtitle">Detects: 0, 3, 5, 8</div>
<div class="card">
<img id="img" src="/img" alt="Camera">
<div id="frame">Frame: 0</div>
</div>
<button class="btn" id="btnDetect" onclick="detect()">Detect Digits</button>
<button class="btn btn-continue" id="btnContinue" onclick="resumeLive()">Continue (Live View)</button>
<div class="card">
<div class="stats">
<div class="stat"><div class="stat-val" id="cnt">0</div>Detections</div>
<div class="stat"><div class="stat-val" id="time">0</div>ms</div>
</div>
<div id="dets"></div>
</div>
<div class="footer">EE4065 Final Project - Question 2<br>YOLO-Tiny Object Detection</div>
</div>
<script>
var n=0;
...
function showLive(){
  if(!showingResult){
    ...
    document.getElementById("frame").innerText="Frame: "+n+" (Live)";
  }
}
...
function detect(){
  ...
  document.getElementById("frame").innerText="Detecting...";
  fetch("/detect").then(function(r){return r.json();}).then(function(d){
    n++;
    document.getElementById("img").src="/result?"+n;
    document.getElementById("frame").innerText="Detection Result - Click Continue to resume";
    document.getElementById("cnt").innerText=d.count;
    document.getElementById("time").innerText=d.time;
    var h="";
    for(var i=0;i<d.detections.length;i++){
      var x=d.detections[i];
      h+="<div class=det><span><b>Digit "+x.digit+"</b></span><span>Box: ["+x.x1+","+x.y1+"]-["+x.x2+","+x.y2+"]</span><span>"+(x.conf*100).toFixed(1)+"%</span></div>";
    }
    if(h.length==0)h="<div style=opacity:0.5;text-align:center;padding:10px>No digits detected</div>";
    document.getElementById("dets").innerHTML=h;
...
function resumeLive(){
  ...
  document.getElementById("btnContinue").style.display="none";
  document.getElementById("frame").innerText="Resuming live view...";
}

liveInterval=setInterval(showLive,1000);
</script>
</body></html>
)rawliteral";
