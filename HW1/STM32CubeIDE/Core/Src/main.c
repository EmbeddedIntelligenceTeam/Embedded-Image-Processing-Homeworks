...
int main(void)
{
  ...
  /* for (int i = 0; i < IMAGE_SIZE; i++) {
   ...
   //2b Tresholding Image Transformation
   ...
   for (int i = 0; i < IMAGE_SIZE; i++) {
       ...
       if (r > threshold_value) {
           output_image[i] = 255; // if bigger than treshold make it white
       } else {
           output_image[i] = 0;   // if smaller than treshold make it black
       }
   }*/


   //2c gama=3.0, gama=1.0/3.0  Qama Correction Transformation
   /*float gamma_value = 1.0/3.0; // Gama value
   ...
   for (int i = 0; i < IMAGE_SIZE; i++) {
       ...
       if (r <= r1) {
           ...
           // Formula: s = (r - r1) * slope + s1
           output_image[i] = (unsigned char)( (float)(r - r1) * slope + (float)s1 );
       }
   }*/

  // Prevent the 'output_image' array from being optimized away.
